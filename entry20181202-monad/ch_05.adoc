...
`.map(f).flat()` のモナドメソッドはendofunctorの上位互換として機能することが確認出来ました。もうこの確定したパターンでは、逐一尻尾に `.flat()` くっつけて回るのは、付け忘れる可能性だってある、スマートではないし、見通しも悪く、バグの温床にもなりかねません。
...
モナドの構成要素となっているこの３つの関数は、3つとも必ずモナドを返す関数であることに注目してください。
====


== モナド関数を設計する

モナド関数は、モナド関数を返せばなんでも自由だという事が判明したので、モナド関数を自由に設計してみます。
...
<1> 9を足す `plus9` というモナド関数を作りたい
<2> ここまで `identity` と一緒　自分自身
<3> 自分自身に `n => unit(n + 9)` というモナド関数で `flatMap`
<4> `plus9` モナド関数で `flatMap`
...
<1> 奇数だけピックアップする `oddFilter` というモナド関数を作りたい
<2> ここまで `identity` と一緒　自分自身
<3> 要素 `n` が奇数のみフィルターするモナド関数を定義していく
<4>  `2`　で割って余りが `1` ならば
<5> 奇数なので、`unit(n)` つまり、要素 `n` 自身 をモナド値として返す
<6> 奇数でないので、`[]` つまり、要素　`n` を削除
<7> `oddFilter` モナド関数で `flatMap`
...
<2> `plus9oddFilter`  モナド関数で `flatMap`
